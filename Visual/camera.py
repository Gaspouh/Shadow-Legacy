...
class Background_effect:
    ...
    def effect(self, fenetre, camera):
        ...
        if self.path_image == os.path.join(Chemin_absolu, "Graphics", "hollow_earth", "Background", "3.png"):
            ...
            offset_y = max(
                -(self.image.get_height() - fenetre.get_height()), min(0, offset_y)
            )  # limiter le déplacement vertical pour ne pas montrer les zones hors limites
...
def draw_luciole(surface, x, y, luciole, now):
    ...
    pygame.draw.circle(
        halo,
        (*luciole["color"], transparence_halo),
        (taille_halo, taille_halo),
        taille_halo,
    )
    ...
    pygame.draw.circle(
        noyau,
        (200, 255, 200, transparence_noyau),
...
def intro(fenetre):
    ...
    texte = [
        "Le monde est tombé dans l’ombre",
        "Les royaumes ont disparu",
        "et la lumière s’est éteinte",
        "Il ne reste que des ruines",
        "et un silence sans fin",
        "Mais une lueur persiste encore",
        "Et cette lueur, c’est toi",
        "va jeune héros et sauve le monde de l'obscurité",
    ]
