...
def home_screen(fenetre):
    ...
    boutton_reprendre = pygame.transform.scale(
        boutton_reprendre,
        (boutton_reprendre.get_width() // 1.4, boutton_reprendre.get_height() // 1.4),
    )
    ...
    boutton_quitter = pygame.transform.scale(
        boutton_quitter,
        (boutton_quitter.get_width() // 3.1, boutton_quitter.get_height() // 3.1),
    )
    ...
    boutton_nouvelle_game = pygame.transform.scale(
        boutton_nouvelle_game,
        (
            boutton_nouvelle_game.get_width() // 1.45,
            boutton_nouvelle_game.get_height() // 1.45,
        ),
    )
    ...
    while running:
        ...
        img, rect, over_reprendre = effet_bouton(
            boutton_reprendre, rect_reprendre, over_reprendre
        )  # on utilise "img" et "rect" pour chaque bouton car dans tout les cas 1 seul bouton à la fois est selectionné
...
def menu(fenetre, player, checkpoints, current_map_name):
    ...
    fenetre.blit(
        image_menu,
        (
            fenetre.get_width() // 2 - image_menu.get_width() // 2,
            fenetre.get_height() // 2 - image_menu.get_height() // 2,
        ),
    )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if bouton_reprendre.collidepoint(event.pos):
                    ...
                    texte = [
                        ...
                        "Espace pour sauter",
                        "Z pour interagir",
                        "E pour l'inventaire quand sur banc",
                        "echapp pour le menu pause",
                        "F pour les attaques à distance",
                        "O pour le soin",
                    ]
                    ...
                    sauvegarder(
                        player, checkpoints, current_map_name, forgeron_instance=None
...
def sit_on_bench(fenetre, player):
    ...
    equip_charms_rect = pygame.Rect(
        fenetre.get_width() - fenetre.get_width() // 3.2,
        fenetre.get_height() - fenetre.get_height() // 2.3,
        450,
        300,
    )  # dessiner un carré pour emplacement des charms equipés
    ...
    for (
        name,
        active,
    ) in found_charms_data.items():  # Pour récupérer valeur et clé de chaque charm et les placer
        if active == True:  # Si charm trouvé c'est True, il est affiché alors
            if name in assets_path:
                image_1 = pygame.image.load(
                    assets_path[name]
                ).convert_alpha()  # Compare avec la liste de path d'image, pour récuperer l'asset
    ...
    for i in range(j):  # montre les réceptacles en les regroupant par 3
        ...
        charms_afficher.append(
            {"img": image2, "rect": rect, "name": "receptacle"}
        )  # les utiliser commme des charmes affichés dans l'inventaire
    ...
    for i in range(player.minerais):
        ...
        charms_afficher.append(
            {"img": image3, "rect": rect, "name": "minerai"}
        )  # les utiliser commme des charmes affichés dans l'inventaire
    ...
    while open_inventory:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if charm_selected:
                    ...
                    if equip_charms_rect.collidepoint(mouse_pos) and charm_selected["name"] not in [
                        i["name"] for i in charms_equiper
                    ]:  # i pour verifier que le charm appartient pas deja a ceux equipés
                        if len(charms_equiper) < 4:  # maximum 4 charms équipés
                            charm_selected["rect"].topleft = (
                                equip_charms_rect.x
                                + 20
                                + len(charms_equiper) * (charm_selected["img"].get_width() + 20),
                                equip_charms_rect.y + 20,
                            )
        ...
        fenetre.blit(
            equipement_text,
            (
                equip_charms_rect.x + equip_charms_rect.width // 2 - equipement_text.get_width() // 2,
                equip_charms_rect.y - 40,
            ),
        )
...
def charms_market(fenetre, sell_charms):
    ...
    orb_img = pygame.transform.scale(pygame.image.load("Assets/Images/orbs.png").convert_alpha(), (45, 45))
    ...
    while True:
        ...
        fenetre.blit(
            orb_text,
            (
                orb_rect.left - orb_text.get_width() - 8,
                orb_rect.centery - orb_text.get_height() // 2,
            ),
        )
