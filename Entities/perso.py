import pygame
import random
from Entities.player_abilities import Dash, Double_jump, sort, soin
...
class Player(PhysicsEntity):
    def __init__(self, x, y, fenetre):
        ...
        self.anim_jump_air_right = VerticalAnimation(
            fenetre, x, y, "Assets/Player/jump_air_right.png", 12, 500, 500, 0, 0
        )
        self.anim_jump_air_left = VerticalAnimation(
            fenetre, x, y, "Assets/Player/jump_air_left.png", 12, 500, 500, 0, 0
        )
        ...
        # attaque
        self.anim_basic_attack1_right = VerticalAnimation(
            fenetre, x, y, "Assets/Player/basic_attack1_right.png", 10, 500, 500, 0, 0
        )
        self.anim_basic_attack2_right = VerticalAnimation(
            fenetre, x, y, "Assets/Player/basic_attack2_right.png", 10, 500, 500, 0, 0
        )
        self.anim_basic_attack1_left = VerticalAnimation(
            fenetre, x, y, "Assets/Player/basic_attack1_left.png", 10, 500, 500, 0, 0
        )
        self.anim_basic_attack2_left = VerticalAnimation(
            fenetre, x, y, "Assets/Player/basic_attack2_left.png", 10, 500, 500, 0, 0
        )
    ...
    def animate(self):
        ...
        if self.current_animation in (self.anim_run_right, self.anim_run_left):
            self.image = pygame.transform.scale(
                frame_surface, (110, 110)
            )  # Perso plus petit lors du run (à cause du pb de "pading" de chaque tiles du spritesheet)
        ...
        if not self.is_stunned:
            ...
            if want_to_jump and self.quicksand_sink < 3: # < 3 pour permettre le saut une fois presque a la surface
                if can_jump:
                    self.execute_jump(
                        jump_type="simple"
                    )  # Exécuter le saut si le jump buffer est actif (c-à-d que le joeur a préssé le bouton de saut) et que le joueur peut sauter (c-à-d que le joueur est dans la fenêtre de coyote time)
            ...
            if self.invincible:
                now = pygame.time.get_ticks()
                if (
                    now - self.invincibility_timer >= self.invincibility_duration
                ):  # Si la durée d'invincibilité est écoulée
                    self.invincible = False
    ...
    def take_damage(self, attack_data, source_rect, source, fade=None):
        ...
        if source.respawn_on_touch:
            ...
            if source_rect.centerx > self.rect.centerx:
                ...
                knockback_direction = 1
            self.velocity.x = (
                attack_data["knockback_x"] * knockback_direction
            )  # Reculer le joueur dans la direction opposée à laquelle il fait face lorsqu'il est touché
