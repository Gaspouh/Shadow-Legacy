...
class NPC_Logic:
    ...
    def __init__(
        ...
        self.dialogue_zone = pygame.Rect(
            self.rect.x - 50,
            self.rect.y - 50,
            self.rect.width + 100,
            self.rect.height + 100,
        )  # zone de déclenchement du dialogue
    ...
    def player_in_dialogue(self, player_rect):
        ...
        Entrées: player_rect.
        Sortie: Retourne True si le joueur est dans la zone de dialogue et que le dialogue n'a pas encore été déclenché, sinon False.
        """
            ...
            if event.button in (1, 3):
                ...
                if self.dialogue_index >= len(self.current_dialogue_list):
                    ...
                    if self.market_seller:
                        self.market_seller(
                            self.fenetre, self.sell_charms
                        )  # market seller est une fonction a modifier dans chaque npc seller
                        # dialogue sortie
    ...
    def draw(self, screen, camera=None):
        ...
        if self.is_speaking and self.dialogue_index < len(self.current_dialogue_list):
            ...
            if self.dialogue_type == "upgrade":
                self.rect_oui = pygame.draw.rect(
                    screen,
                    (0, 200, 0),
                    (screen_pos[0] - 60, screen_pos[1] - 20, 50, 30),
                )
                self.rect_non = pygame.draw.rect(
                    screen,
                    (200, 0, 0),
                    (screen_pos[0] + 10, screen_pos[1] - 20, 50, 30),
                )
                font = pygame.font.SysFont("Arial", 12, bold=True)
                screen.blit(
                    font.render("OUI", True, (255, 255, 255)),
                    (screen_pos[0] - 50, screen_pos[1] - 12),
                )
                screen.blit(
                    font.render("NON", True, (255, 255, 255)),
                    (screen_pos[0] + 20, screen_pos[1] - 12),
                )
...
class Forgeron(NPC_Logic, Animation):
    def __init__(self, fenetre, x, y):
        ...
        self.leave_dialogue = ["À bientôt, prends garde aux ténèbres qui rôdent dans ce monde."]
    ...
    def dialogue_equipement(self):
        ...
        self.upgrade_dialogue = [
            ...
            + " minerais et "
            + str(self.orb_cost)
            + " pièces. Veux-tu procéder à l'amélioration ?"
        ]
    ...
    def confirm_upgrade(self, player):
        ...
        ):  # Vérifie si le joueur a les ressources nécessaires pour l'amélioration
            self.upgrade(player)  # Effectue l'amélioration de l'équipement du joueur

        else:
            self.start_dialogue(
                ["Désolé, tu n'as pas les ressources nécessaires pour l'amélioration."],
                dialogue_type="normal",
            )  # Dialogue si le joueur n'a pas les ressources nécessaires
    ...
    def upgrade(self, player):
        ...
        self.start_dialogue(
            ["Ton arme a été améliorée , tu es maintenant plus fort contre les ennemis !"],
            dialogue_type="normal",
        )  # Dialogue de confirmation de l'amélioration
