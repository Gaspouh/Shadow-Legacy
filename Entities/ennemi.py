...
class Patrouilleur(Ennemi):
    ...
    def patrouille(self, platforms):
        ...
        if self.on_ground:
            ...
            self.image = (
                self.frames_droite[int(self.index_image)]
                if self.direction == 1
                else self.frames_gauche[int(self.index_image)]
            )
...
class Araignee(Patrouilleur):
    def __init__(self, fenetre, x, y):
        super().__init__(
            ...
            x,
            y,
            "Assets/Images/insecte_sheet2.png",
            8,
            70,
            50,
            13,
            ...
            3,
            1.7,
            {"damage": 1, "knockback_x": 80, "knockback_y": -4},
            scale=1,
            reward=4,
        )
...
class Volant(Ennemi):
    ...
    def poursuite(self, player_rect, platforms):
        ...
        Animation.gestion_animation(self)
        if (
            abs(player_rect.x - self.rect.x) < 300 and abs(player_rect.y - self.rect.y) < 300
            ...
            self.image = (
                self.frames_gauche[int(self.index_image)]
                if self.direction == 1
                else self.frames_droite[int(self.index_image)]
            )
    ...
    def tir(self, player_rect, tir_tourelle):
        ...
        if self.dans_trigger(player_rect, trigger_range=300) and self.coldown == 0 and self.oriente:
            ...
            projectile = Projectile(
                ...
                height=20,
                damage=1,
                image=pygame.image.load("Assets/Images/tir_tourelle.png").convert_alpha(),
            )
...
class Fighter(Ennemi):
    def __init__(self, fenetre, x, y):
        ...
        super().__init__(
            ...
            10,
            1.5,
            {"damage": 2, "knockback_x": 40, "knockback_y": -4},
            scale=1,
            reward=5,
        )
    ...
    def mouvement(self, player_rect, player, platforms):
        ...
        if self.dans_trigger(player_rect, trigger_range=500):
            ...
            if (
                abs(player_rect.centerx - self.rect.centerx) < 80 and abs(player_rect.centery - self.rect.centery) < 50
        ...
        if self.attacking:
            if self.hitbox:
                if self.hitbox.rect.colliderect(player.rect):
                    if (
                        self.hitbox not in player.entite_touches
                        ...
                        hitstop_duration, shake_amount = player.take_damage(
                            self.attack_data, self.rect, self
                        )  # Appliquer les effets de l'attaque au joueur
    ...
    def attaque(self):
        ...
        if self.cooldown == 0:
            ...
            self.hitbox.rect.center = (
                self.rect.centerx + (80 * self.direction),
                self.rect.centery,
            )  # Positionner la hitbox devant le fighter en fonction de sa direction
            return self.hitbox
...
class Chargeur(Ennemi):
    def __init__(self, fenetre, x, y):
        super().__init__(
            ...
            3,
            3,
            {"damage": 1, "knockback_x": 150, "knockback_y": -4},
            scale=1,
            reward=8,
        )
    ...
    def charge(self, player_rect, platforms):
        ...
        if self.attacking:
            ...
            if self.dans_trigger(player_rect, trigger_range=400) and not self.attacking:
                ...
                if (
                    abs(player_rect.centerx - self.rect.centerx) < 300
                    and abs(player_rect.centery - self.rect.centery) < 200
                ):  # Si le joueur est à portée de charge
                    self.charge_timer += 1
