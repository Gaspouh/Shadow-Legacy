...
class Ennemi(Animation, PhysicsEntity):
    ...
    def mort(self):
        if not self.alive:
            if not self.dead:
                self.dead = True
                self.animation_mort.index_image = 0  # remettre à zéro au début

            # Jouer l'animation de mort une seule fois et récupérer si elle est terminée
            _, fin = self.animation_mort.gestion_animation_once()
...
class Tourelle(Ennemi):
    def __init__(self, fenetre, x, y):
        ...
        super().__init__(fenetre, x, y, 'Assets/Images/tourelle.png', 8, 70, 63, 0, 0, 1, 1.7, {"damage": 1, "knockback_x": 80, "knockback_y": -4}, scale=1)
...
class Chargeur(Ennemi):
    def __init__(self, fenetre, x, y):
        super().__init__(fenetre, x, y, 'Assets/Images/chargeur.png', 8, 82, 58, 3, 0, 3, 3, {"damage": 1, "knockback_x": 150, "knockback_y": -4}, scale=1)
