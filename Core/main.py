...
map_manager.load_map(
    os.path.join(Chemin_absolu, "Graphics", current_map_name, current_map_path)
)  # charge la map actuelle
layers = create_parallax_layers(
    os.path.join(Chemin_absolu, "Graphics", current_map_name),
    nb_layers=map_manager.nb_parallax_layers,
    fenetre=game_fenetre,
)  # créer les layers de parallax pour la map actuelle
...
spawn_point, cadavre_data, spawn_map = charger(
    player, checkpoints, current_map_name, forgeron_instance
)  # charge la save si elle existe, sinon spawn par défaut + le cadavre
...
for obj in objects:
    objet_data = {"map": current_map_name, "x": obj.rect.x, "y": obj.rect.y}
...
while continuer:
                ...
                if event.button == 1:  # Clic gauche
                    ...
                    for bot in pnj:
                        if bot.is_speaking:
                            ...
                            player.press_attack()
                    if (
                        not pnj_en_train_de_parler
                    ):  # Si on n'est pas en train de parler à un PNJ, on attaque normalement
                        # faire attaquer le joueur
                        player.press_attack()
    ...
    if player.health > 0:
        ...
        if now > hitstop_until:
            ...
            player.update(
                proche + special_surfaces
            )  # Mettre à jour le joueur avec les plateformes pour gérer les collisions
            ...
            for e in liste_entites[:]:
                ...
                if e.rect.colliderect(player.rect):
                    hitstop_duration, shake_amount = player.take_damage(
                        e.attack_data, e.rect, e
                    )  # Appliquer les effets de recul au joueur si un ennemi le touche
            ...
            for e in liste_entites:
                if player.is_attacking and player.attack_rect.colliderect(e.rect) and e.alive:
                    if (
                        e not in player.entite_touches
                    ):  # Vérifier que cet ennemi n'a pas déjà été touché par cette attaque
                        ...
                        e.receive_hit(
                            player.attack_data, player.rect, player
                        )  # Appliquer les effets de recul à l'ennemi
            ...
            for platform in platforms:
                if platform.rect.colliderect(rendu_visible):  # blit si c'est ds la partie visible
                    game_fenetre.blit(
                        platform.image, camera.apply(platform.rect)
                    )  # Appliquer le décalage de rendu pour le screen shake
            ...
            for trap in traps:
                ...
                if player.is_attacking and player.attack_rect.colliderect(trap.rect):
                    if (
                        trap not in player.entite_touches
                    ):  # Vérifier que ce piège n'a pas déjà été touché par cette attaque
            ...
            for obj in objects[:]:
                ...
                obj.draw_big(
                    game_fenetre, player
                )  # Afficher les objets pris en grand pour indiquer qu'ils ont été ramassés
        ...
        # Joueur
        image_rect = player.image.get_rect(
            midbottom=(
        ...
        if (
            not player.invincible or (pygame.time.get_ticks() // 100) % 2 == 0
        ):  # Pour faire clignoter le joueur quand il est invincible
            game_fenetre.blit(player.image, camera.apply(image_rect))

        if player.is_attacking and pygame.key.get_pressed()[pygame.K_a]:
            pygame.draw.rect(
                game_fenetre, (255, 0, 0), camera.apply(player.attack_rect)
            )  # Afficher la hitbox de l'attaque pour les tests
        if pygame.key.get_pressed()[pygame.K_a]:
            pygame.draw.rect(
                game_fenetre, (0, 0, 255), camera.apply(player.rect), 2
            )  # Afficher la hitbox de l'attaque pour les tests
        ...
        for heart in hearts:
            game_fenetre.blit(
                heart.image, heart.rect
            )  # Les cœurs sont fixes à l'écran, pas besoin d'appliquer le décalage de la caméra
        ...
        for elem in tir_tourelle[:]:  # On parcourt la liste des projectiles tirés par les tourelles
            ...
            if elem.rect.colliderect(player.rect) and not elem.hit:  # Si un projectile de tourelle touche le joueur
                ...
                hitstop_duration, shake_amount = player.take_damage(
                    elem.attack_data, elem.rect, elem
                )  # Appliquer les dégâts et le recul au joueur
                hitstop_until = pygame.time.get_ticks() + hitstop_duration  # Activer le hitstop

    # Gestion de mort
    else:
        cadavre = Cadavre(
            last_pos[0], last_pos[1], Monnaie.orbs, current_map_name
        )  # créer le  cadavrte a la dernire pos
        Monnaie.orbs = 0
        sauvegarder(
            player,
            checkpoints,
            ...
            cadavre=cadavre,
        )  # sauvegarder la position du cadavre
        ...
        spawn_point = (
            pygame.math.Vector2(main_spawn["x"], main_spawn["y"]) if main_spawn else spawn_point
        )  # reset avec le bon spawn point
