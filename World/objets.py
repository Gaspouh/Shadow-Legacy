...
class Monnaie:
    pygame.mixer.init()
    orbs = 0 # Globale
    add_sound = pygame.mixer.Sound("Assets/Sounds/add_orbs.mp3")
    def __init__(self, fenetre, x, y):
        self.fenetre = fenetre
        self.image = pygame.image.load("Assets/Images/orbs.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (55, 55))
    ...
    def draw(self, fenetre):
        ...
        for i in range(-8, 9, 3):
            for j in range(-8, 9, 3):
                glow_surf = self.font.render(texte, True, (210, 225, 255)) # version plus "bleu"
                glow_surf.set_alpha(11) # Fondue plus moins forte
                fenetre.blit(glow_surf, (self.rect.left - glow_surf.get_width() - 8 + i,
                    self.rect.centery - glow_surf.get_height() // 2 + j))
